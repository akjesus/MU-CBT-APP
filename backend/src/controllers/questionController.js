...
exports.getQuestionById = async (req, res) => {
    try {
        const [question] = await db.query("SELECT * FROM questions WHERE id = ?", [req.params.id]);
...
exports.createQuestion = async (req, res) => {
    try {
        ...
        const [result] = await db.query(
            ...
            [course_id, text, option_a, option_b, option_c, option_d, correct_option, difficulty_level, question_type, score_obtainable, level, file, answers, user_id]
        );
        res.status(201).json({ message: "Question added to Question Bank", id: result.insertId });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
exports.addQuestionToExam = async (req, res) => {
    try {
        ...
        await db.query("INSERT INTO exam_questions (exam_id, question_id, created_at, updated_at) VALUES (?, ?, NOW(), NOW())", [examId, question_id]);
