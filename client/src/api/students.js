...
export const updateStudent = (data) => {
    const token = localStorage.getItem('token');
    return api.put(`/students/${data.id}`, {
        first_name: data.first_name,
        last_name: data.last_name,
        email: data.email,
        matric: data.matric,
...
export const changePassword = (data) => {
    const token = localStorage.getItem('token');
    return api.post(`${BASE_URL}/students/change-password`,
        {
...
const token = localStorage.getItem('token');
    return api.post(`/students/update-picture`,
        formData,
        {
            headers: {
                'Authorization': `Bearer ${token}`
...
export const blockUnblockStudent = (id, isBlocked) => {
    const token = localStorage.getItem('token');
    return api.put(`/students/${id}/block-unblock`, { isBlocked }, {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        }
